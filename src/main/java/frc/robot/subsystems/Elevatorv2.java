...
public class Elevatorv2 extends SubsystemBase {
  ...
  ProfiledPIDController controller = new ProfiledPIDController(1,0,0, new Constraints(maxVelocity, maxAcceleration));
  ...
  TalonFXSimState simState = rightMotor.getSimState();
  ...
  public Elevatorv2() {
    ...
    slot0Configs.kV = m_elevatorPlant.getA(1, 1)/m_elevatorPlant.getB(1,0); // A velocity target of 1 rps results in 0.12 V output
    slot0Configs.kA = 1/m_elevatorPlant.getB(1,0); // An acceleration of 1 rps/s requires 0.01 V output
    ...
    //Logging
    LogManager.logSupplier("Elevator/Voltage", () -> getVoltage(), 100, LogLevel.DEBUG);
    LogManager.logSupplier("Elevator/Velocity", () -> getVelocity(), 100, LogLevel.DEBUG);
    LogManager.logSupplier("Elevator/position", () -> getPosition(), 100, LogLevel.DEBUG);
