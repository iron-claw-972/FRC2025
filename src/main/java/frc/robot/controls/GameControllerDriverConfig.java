...
// Elevator setpoints
 ...
 // Intake/outtake
        ...
        if(outtake != null && elevator != null){
            kDriver.get(DPad.DOWN).and(menu.negate()).onTrue(new OuttakeCoral(outtake, elevator, arm).alongWith(new InstantCommand(()->getDrivetrain().setDesiredPose(()->null))).
            ...
            kDriver.get(DPad.DOWN).and(menu.negate()).onTrue(new InstantCommand(()->{}, getDrivetrain()));
        if(intake != null && indexer != null){
         ...
         // Climb
        if(climb != null){
            kDriver.get(Button.X).and(menu.negate()).toggleOnTrue(new StartEndCommand(()->climb.extend(), ()->climb.climb(), climb));
            if(intake != null){
